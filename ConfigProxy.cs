...
public partial class ConfigProxy
{
    ...
    private static byte[] ModifyResponsePayload(byte[] payload, string endpoint)
    {
        ...
        if (baseEndpoint == "/api/v1/config/public")
        {
            ...
            if (configObject?["keystone.client.feature_flags.restart_required.disabled"] is not null)
            {
                configObject["keystone.client.feature_flags.restart_required.disabled"] = true;
            }

            if (configObject?["keystone.client.feature_flags.vanguardLaunch.disabled"] is not null)
            {
                configObject["keystone.client.feature_flags.vanguardLaunch.disabled"] = true;
            }
            ...
            if (configObject?["lol.client_settings.vanguard.enabled_embedded"] is not null)
            {
                configObject["lol.client_settings.vanguard.enabled_embedded"] = false;
            }

            if (configObject?["lol.client_settings.vanguard.url"] is not null)
            {
                configObject["lol.client_settings.vanguard.url"] = "";
            }
            ...
            var pbe = configObject?["keystone.products.league_of_legends.patchlines.pbe"];
            if (pbe is not null)
            {
                var configs = pbe["platforms"]?["win"]?["configurations"]?.AsArray();
                if (configs is not null)
                {
                    for (var i = 0; i < configs.Count; ++i)
                    {
